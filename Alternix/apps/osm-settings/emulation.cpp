...
{
    // Waydroid is not in Devuan/Debian main below Debian 14. On excalibur it
    // lives in excalibur-backports; the waydro.id third-party repo is the
    // fallback. That repo's setup script rejects Devuan codenames outright,
    // so the Debian equivalent has to be passed explicitly.
    const QString cmd = QString(SUDO_PREAMBLE) +
        "CODENAME=$(. /etc/os-release 2>/dev/null; echo \"$VERSION_CODENAME\")\n"
        "case \"$CODENAME\" in\n"
        "  chimaera)  DEB=bullseye ;;\n"
        "  daedalus)  DEB=bookworm ;;\n"
        "  excalibur) DEB=trixie   ;;\n"
        "  freia)     DEB=forky    ;;\n"
        "  *)         DEB=\"$CODENAME\" ;;\n"
        "esac\n"
        "echo \"Distribution: ${CODENAME:-unknown} (Debian ${DEB:-unknown})\"\n"
        "\n"
        "$SUDOX env DEBIAN_FRONTEND=noninteractive apt-get update\n"
        "\n"
        "# 1. already available?\n"
        ...
        "fi\n"
        "\n"
        "# 2. backports (Debian 13 / Devuan excalibur ships it here)\n"
        "if [ -n \"$CODENAME\" ]; then\n"
        "  echo \"Enabling ${CODENAME}-backports...\"\n"
        "  echo \"deb http://deb.devuan.org/merged ${CODENAME}-backports main\" "
        "| $SUDOX tee /etc/apt/sources.list.d/backports.list >/dev/null\n"
        "  $SUDOX env DEBIAN_FRONTEND=noninteractive apt-get update\n"
        "  if apt-cache show waydroid >/dev/null 2>&1; then\n"
        "    $SUDOX env DEBIAN_FRONTEND=noninteractive apt-get -y "
        "-t \"${CODENAME}-backports\" install waydroid\n"
        "    exit 0\n"
        "  fi\n"
        "  echo \"Not in backports either; trying the Waydroid repository...\"\n"
        "fi\n"
        "\n"
        "# 3. official third-party repo\n"
        "if [ -z \"$DEB\" ]; then\n"
        "  echo \"ERROR: could not work out which Debian release this is based on.\"\n"
        "  exit 1\n"
        "fi\n"
        "if command -v curl >/dev/null 2>&1; then\n"
        "  curl -fsSL --proto '=https' --tlsv1.2 https://repo.waydro.id/waydroid.gpg "
        "| $SUDOX tee /usr/share/keyrings/waydroid.gpg >/dev/null\n"
        "elif command -v wget >/dev/null 2>&1; then\n"
        "  wget -qO- https://repo.waydro.id/waydroid.gpg "
        "| $SUDOX tee /usr/share/keyrings/waydroid.gpg >/dev/null\n"
        "else\n"
        "  echo \"ERROR: neither curl nor wget is installed.\"\n"
        "  exit 1\n"
        "fi\n"
        "echo \"deb [signed-by=/usr/share/keyrings/waydroid.gpg] https://repo.waydro.id/ $DEB main\" "
        "| $SUDOX tee /etc/apt/sources.list.d/waydroid.list >/dev/null\n"
        "$SUDOX env DEBIAN_FRONTEND=noninteractive apt-get update\n"
        "if ! apt-cache show waydroid >/dev/null 2>&1; then\n"
        "  echo \"ERROR: no waydroid package after adding the Waydroid repository for '$DEB'.\"\n"
        "  exit 1\n"
        "fi\n"
        "$SUDOX env DEBIAN_FRONTEND=noninteractive apt-get -y install waydroid\n";
